...
describe("Visual Novelty Agent & Cooldown Tests", () => {
  it("should correctly map engineering problems to appropriate visual types", () => {
    ...
    expect(visualNoveltyAgent.mapProblemToVisualType(t1.title, t1.category)).toBe(VisualType?.BENCHMARK_CHART || "BENCHMARK_CHART");
  ...
  it("should select a fresh visual type if the preferred type conflicts with recent visual history", () => {
    const history: VisualRecord[] = [
      {
        ...
        visualType: (VisualType?.BENCHMARK_CHART || "BENCHMARK_CHART") as any, // benchmark chart was just used
    ...
    expect(selected).not.toBe(VisualType?.BENCHMARK_CHART || "BENCHMARK_CHART");
  });
  ...
  it("should reject visual design if visual novelty score falls below the required threshold due to multiple collisions", () => {
    ...
    const record: VisualRecord = {
      ...
      visualType: (VisualType?.ARCHITECTURE_DIAGRAM || "ARCHITECTURE_DIAGRAM") as any,
      layout: "DYNAMIC_VECTOR_FLOW",
      subject: testTopic.title,
      prompt: "prompt text",
      visualHash: `${testTopic.title.toLowerCase().replace(/[^a-z0-9]/g, "")}_${VisualType?.ARCHITECTURE_DIAGRAM || "ARCHITECTURE_DIAGRAM"}`,
      createdAt: new Date().toISOString(),
    };
